...
public class Mover : MonoBehaviour
{
	...
	[SerializeField] bool lookAtDirection = false;
	[Tooltip("Does the object move according to the local or world system")]
	[SerializeField] bool isMovementLocal = false;
	...
	[SerializeField] Vector3 wallAvoidanceSize = new Vector3(0.3f, 1f, 0.1f);
	[Tooltip("Local offset of wall detection")]
	[SerializeField] Vector3 wallAvoidanceOffset = new Vector3(0, 0, 0.5f);
	[Tooltip("Which Layers are considered a Wall ?")]
	[SerializeField] LayerMask wallAvoidanceLayers = 1;
	...
	[SerializeField] string speedParameterName = "MoveSpeed";
	...
	[SerializeField] float collisionCheckRadius = 0.65f;
	...
	private void Update ()
	{
		if(movementType == MovementType.Rigidbody) return;
		if (!canMove) return;
	...
	private void HandleParticleEmission()
	{
		...
		hvMagnitude = movementType == MovementType.Rigidbody
	...
	private void OnDrawGizmosSelected ()
	{
		if(displayDebugInfo)
		{
			...
			{
				Gizmos.color =
					Helper.GroundCheck(transformPosition + collisionOffset, collisionCheckRadius, groundLayerMask)
						? Color.blue
						: Color.red;
	...
	void SetIdleValues(string parameterName)
	{
	...
	private void UpdateAnimatorValues (Vector3 speed)
	{
		...
		bool isGrounded = Helper.GroundCheck(transform.position + collisionOffset, collisionCheckRadius, groundLayerMask);
